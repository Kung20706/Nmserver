...
func usage(exitCode int, extraMessage ...interface{}) {
	...
	📌  舉例： PROJECT_ENV=local ./app server
	📌  舉例： PROJECT_ENV=local ./app schedule
	📌  舉例： PROJECT_ENV=local ./app run %s
...
func Run(payload ...func()) {
	if bootstrap.GetAppEnv() == "" || bootstrap.GetAppSite() == "" {
		usage(0)
	} else {
		bootstrap.WriteLog("INFO", fmt.Sprintf("⚙  PROJECT_ROOT: %s", bootstrap.GetAppRoot()))
		bootstrap.WriteLog("INFO", fmt.Sprintf("⚙  PROJECT_ENV: %s", bootstrap.GetAppEnv()))
		bootstrap.WriteLog("INFO", fmt.Sprintf("⚙  PROJECT_SITE: %s", bootstrap.GetAppSite()))
	}
	args := os.Args
	if len(args) < 2 {
		usage(0)
